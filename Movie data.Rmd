...
```{r,message=FALSE}
budgetmod <- lm(tmdb_enhanced$revenue ~ tmdb_enhanced$budget)
budgetr <- tmdb_enhanced %>% 
  ggplot(aes(x=budget, y=revenue)) +
  geom_point(color="red") +
  geom_abline(intercept = -2.630e+06, slope = 2.923e+00, size = 1)

popularitymod <- lm(tmdb_enhanced$revenue ~ tmdb_enhanced$popularity)
popularityr <- tmdb_enhanced %>% 
  ggplot(aes(x=popularity, y=revenue)) +
  geom_point(color="red") +
  geom_abline(intercept = 11334117, slope = 3300090, size = 1)


votecountmod <- lm(tmdb_enhanced$revenue ~ tmdb_enhanced$vote_count)
votecountr <- tmdb_enhanced %>% 
  ggplot(aes(x=vote_count, y=revenue)) +
  geom_point(color="red") +
  geom_abline(intercept = 11107575, slope = 103088, size = 1)

voteaveragemod <- lm(tmdb_enhanced$revenue ~ tmdb_enhanced$vote_average)
voteaverager <- tmdb_enhanced %>% 
  ggplot(aes(x=vote_average, y=revenue)) +
  geom_point(color="red") +
  geom_abline(intercept = -81476779, slope = 26876692, size = 1)

runtimemod <- lm(tmdb_enhanced$revenue ~ tmdb_enhanced$runtime)
runtimer <- tmdb_enhanced %>% 
  ggplot(aes(x=runtime, y=revenue)) +
  geom_point(color="red") +
  geom_abline(intercept = -111013532, slope = 1808719, size = 1)
...
```{r}
basicmod <- lm(revenue ~ budget + popularity + runtime + vote_average + vote_count + genre1,data = tmdb_enhanced)
summary(basicmod)
```
...
```{r}
summary(basicmod$fitted.values)
```
