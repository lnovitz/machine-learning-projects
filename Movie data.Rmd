...
This dataset includes around 5000 movies and their relative variables. Our group analyzes the variables in it and biuld up two models to explain what factors would influence the revenue of a movie using forward selection and LASSO methods. 
...
summary(basicmod)
```
From the model, we see that the p-value of budget, popularity, runtime, and vote_count are all significant, but vote_average is not significant. And for the categorical variable, some of them are significant while others are not. So, we decide to exclude vote_average. And then, we plot the residual vs. fitted value, histogram and qq plots to check the model assumtion. 
...
Next, we calculate the cv errors of different models in order to find a best model. We are limiting the most number of variables to 10, because more than 10 variables makes the model too complicated. 
...
Through this model, we can conclude that a increase in  budget, popularity, runtime and vote_count will increase the predicted revenue of a movie. And if the category of the movie is Animation or Family, it tends to increases the revenue of the movie dramatically, while the Adventure, Comedy, Horror and Romance categories also increases the predicted revenue of a movie. 

### Part V: Choose three best explainatory varibles. 

In this section, we want to find out three variables with the largest interpretability. We want to see which variables decide the predicted renenue of a movie most. 
We use LASSO method to decreases the number of variable until three left to find this variable. 
...
We tried 4 different lambda values, which are the lambda that minimize the cv error, $1*10^7$,$(1.95)*10^7$, and the lambda that 1 standard error from the min lambda. As the result, when lambda = $(1.95)*10^7$, we have three variables left. The left variables are budget, popularity and vote_count. This means that the genre of a movie and other variables do not influence the predicted revenue of a movie as much as its budget, popularity and number of votes. Therefore, a movie with a high budget tends to have high revenue. And the movie with high popularity and thus more votes in websides tend to have higher revenue. The final model we got is:
