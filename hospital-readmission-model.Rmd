...
```{r}
x <- model.matrix(readmitted ~., data=train2 %>% select(-grp))[,-1]
y <- train2$readmitted
...
set.seed(1)
cv.lasso.mod <- cv.glmnet(x = x, y = y, alpha = 1, family="binomial")
...
predict(cv.lasso.mod, 
        type="coefficients", 
        s = c(cv.lasso.mod$lambda.min, cv.lasso.mod$lambda.1se, 0.01))
```
